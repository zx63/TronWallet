...
//    txtPath = System.getProperty("user.dir") + '/' + config.getString("CityDb.TxtPath");
    ...
    String fullNode = org.tron.MyUtils.Config.FULL_NODE;
    String solidityNode = org.tron.MyUtils.Config.SOLIDITY_NODE;
...
//    }
  ...
  public static WalletClient GetWalletByStorage(String password) {
    EntityPassword entityPassword = SQLiteUtil.getEntityPassword();
    if(entityPassword == null) {
      return null;
    }

    byte[] priKeyEnced = Hex.decode(entityPassword.privKeyEnced);//loadPriKey();
    if (ArrayUtils.isEmpty(priKeyEnced)) {
      return null;
    }
    //dec priKey
    byte[] salt0 = Hex.decode(entityPassword.salt0);//loadSalt0();
  ...
  public void store(String password) {
    ...
    // SAVE PASSWORD
    EntityPassword entityPassword = new EntityPassword();
    entityPassword.salt0 = Hex.toHexString(salt0);
    entityPassword.salt1 = Hex.toHexString(salt1);
    entityPassword.password = Hex.toHexString(pwd);
    entityPassword.pubKey = Hex.toHexString(pubKey);
    entityPassword.privKeyEnced = Hex.toHexString(privKeyEnced);

    EntityAccount entityAccount = new EntityAccount();
    entityAccount.pubKey = Hex.toHexString(pubKey);
    entityAccount.priKey = Hex.toHexString(privKeyEnced);
    entityAccount.address = WalletClient.encode58Check(ecKey.getAddress());

    SQLiteUtil.setPasswordEntity(entityPassword);
    SQLiteUtil.setAccountEntity(entityAccount);
...
//    FileUtil.saveData(FilePath, walletData);
  ...
  public GrpcAPI.Return sendCoin(byte[] to, long amount) {
  ...
  public GrpcAPI.Return updateAccount(byte[] accountNameBytes) {
    byte[] owner = getAddress();
  ...
  public GrpcAPI.Return updateAsset(byte[] description, byte[] url, long newLimit, long newPublicLimit) {
    byte[] owner = getAddress();
  ...
  public GrpcAPI.Return transferAsset(byte[] to, byte[] assertName, long amount) {
    byte[] owner = getAddress();
  ...
  public GrpcAPI.Return participateAssetIssue(byte[] to, byte[] assertName, long amount) {
    byte[] owner = getAddress();
  ...
  public static GrpcAPI.Return broadcastTransaction(byte[] transactionBytes)
  ...
  public GrpcAPI.Return createAssetIssue(Contract.AssetIssueContract contract) {
  ...
  public GrpcAPI.Return createWitness(byte[] url) {
    byte[] owner = getAddress();
  ...
  public GrpcAPI.Return updateWitness(byte[] url) {
    byte[] owner = getAddress();
  ...
  public GrpcAPI.Return voteWitness(HashMap<String, String> witness) {
    byte[] owner = getAddress();
    Contract.VoteWitnessContract contract = createVoteWitnessContract(owner, witness);
  ...
  public static Transaction createTransaction4Transfer(Contract.TransferContract contract) {
    ...
    Transaction.raw rawData = transaction.getRawData().toBuilder().setExpiration(expiration).setTimestamp(System.currentTimeMillis()).build();
...
//  }
  ...
  public static boolean checkPassWord(String password) {
    ...
    byte[] salt1 = Hex.decode(entityPassword.salt1);//loadSalt1();
    if (ArrayUtils.isEmpty(salt1)) {
      return false;
    }
    byte[] pwd = getPassWord(password, salt1);
    byte[] pwdStored = Hex.decode(entityPassword.password);//loadPassword();
...
//  }
  ...
  public GrpcAPI.Return freezeBalance(long frozen_balance, long frozen_duration) {
  ...
  public GrpcAPI.Return unfreezeBalance() {
  ...
  public GrpcAPI.Return unfreezeAsset() {
  ...
  public GrpcAPI.Return withdrawBalance() {
